...
export default class UserMgt extends React.Component {
    ...
    handleDeleteUser(event) {
        actions.deleteUser(
            $(event.target).parent().find("span:first").text()
        );
    }
        ...
        return (
            <div className="user-mgtbox">
                <AddUserModal
                    ...
                    closeAddModalEvent = {this.closeAddModal}
                    addUserEvent={this.addUser}/>
                <ToolsBar 
                    filterText={this.state.filterText}
                    filterTextChange={this.handleChange}
                    addUserEvent={this.openAddModal}/>
                <div className="user-box w700">
                    <span>ID</span>
                    <span>名字</span>
                    <span>地址</span>
                    <span>操作</span>
